import { defaultSetting, ISvgComponent } from "@/assets/icons/config";
const SearchSvg = ({
  fill = defaultSetting.fill,
  width = defaultSetting.width,
  height = defaultSetting.height,
}: ISvgComponent) => {
  return (
    <svg
      width={width}
      height={height}
      viewBox="0 0 34 34"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
    >
      <mask id="mask0_2_120" style={{ maskType: "alpha" }} maskUnits="userSpaceOnUse" x="0" y="0" width="34" height="34">
        <rect width="34" height="34" fill="url(#pattern0_2_120)" />
      </mask>
      <g mask="url(#mask0_2_120)">
        <rect x="-13" y="-13" width="60" height="60" rx="20" fill={fill} />
      </g>
      <defs>
        <pattern id="pattern0_2_120" patternContentUnits="objectBoundingBox" width="1" height="1">
          <use xlinkHref="#image0_2_120" transform="scale(0.0104167)" />
        </pattern>
        <image
          id="image0_2_120"
          width="96"
          height="96"
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
};

export default SearchSvg;
